...
```bash
# 1. 取得程式碼（策略 DonchianBreakout / NostalgiaForInfinityX7 / docker-compose.yml / Makefile 都在 git 裡）
git clone <你的 repo> freqtrade-okx && cd freqtrade-okx

# 2. 傳上唯二沒進 git 的機密 config（含 UI 帳密 / jwt_secret_key）
scp user_data/config.json user_data/config.nfi.json \
    <user>@<vps>:~/freqtrade-okx/user_data/

# 3. ⚠️ 關鍵：官方 image 以 uid 1000 執行，要讓它能寫 user_data（否則 sqlite 寫不了、bot 報錯）
sudo chown -R 1000:1000 user_data

# 4. 啟動雙 dry-run bot（donchian 8080 + nfi 8081）
...
- `docker-compose.yml`（在 git）
- `user_data/strategies/DonchianBreakout.py`、`NostalgiaForInfinityX7.py`（皆在 git）
- `user_data/config.json`、`user_data/config.nfi.json`（**含機密，不在 git**，手動 scp）

最簡單做法 —— git clone 後補上 2 個 git 不帶的機密 config：

```bash
git clone <你的 repo> freqtrade-okx && cd freqtrade-okx
# 兩個機密 config，從本機傳上去：
scp user_data/config.json user_data/config.nfi.json \
    <user>@<vps-ip>:~/freqtrade-okx/user_data/
```

> `config.json` / `config.nfi.json` 被 `.gitignore` 排除（含 `jwt_secret_key` 與 UI 帳密），所以要手動傳，
> 否則 bot 起不來。公網部署前**務必**換掉兩個 config 裡的 `username` / `password` / `jwt_secret_key`（目前是練習用弱值）。
...
兩個 bot 各有獨立 Web UI：
- **8080** — DonchianBreakout（帳密見 `config.json`）
- **8081** — NostalgiaForInfinityX7（帳密見 `config.nfi.json`）

下面以 8080 為例，**8081 同理**（防火牆與 CORS 兩個 port 都要設）。兩種連法擇一。
...
1. **雲端防火牆限來源 IP**（網路層，決定「誰連得到」）
   - Lightsail：實例 → Networking → IPv4 Firewall，為 TCP `8080` **和 `8081`** 把 Source 從
     `Anywhere (0.0.0.0/0)` 改成 `Restricted to IP`，填你的固定對外 IP。
   - 沒設這步 = port 對全世界開放，弱密碼裸奔。

2. **CORS 加公網來源**（應用層，決定「瀏覽器請求被不被接受」）
   `config.json`（8080）與 `config.nfi.json`（8081）的 `api_server.CORS_origins` 都要加公網來源。
   注意：用哪個 UI 連對方就要把那個來源也加進去（例如用 8080 的 UI 連 8081，8081 的 CORS 要含 `:8080`）。
   ```json
   "CORS_origins": [
       "http://<vps-ip>:8080",
       "http://<vps-ip>:8081"
   ...
   - `127.0.0.1` / `localhost` 那幾條純公網存取用不到，但留著當後路（之後想走 SSH tunnel 免改）。
   - 本機目前已加 `18.178.208.252` 的 8080/8081 來源作為參考範例。
...
把 `docker-compose.yml` 的 `8080:8080` / `8081:8081` 改回 `127.0.0.1:8080:8080` / `127.0.0.1:8081:8081`，
`docker compose up -d` 後，從本機開隧道（一條命令同時轉兩個 port）：

```bash
ssh -L 8080:localhost:8080 -L 8081:localhost:8081 <user>@<vps-ip>
```

維持連線時瀏覽器開 `http://localhost:8080`（主）/ `http://localhost:8081`（NFI）。
此法不需動防火牆 / CORS（既有的 localhost 白名單已涵蓋）。
