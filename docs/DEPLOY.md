...
三個 bot 各有自己的 port：8080 DonchianBreakout / 8081 EmaTrendFollow / 8082 VolBreakout，
...
1. **雲端防火牆限來源 IP**（網路層，決定「誰連得到」）
   - Lightsail：實例 → Networking → IPv4 Firewall，為 TCP `8080-8082` 把 Source 從
     `Anywhere (0.0.0.0/0)` 改成 `Restricted to IP`，填你的固定對外 IP。
   - 沒設這步 = port 對全世界開放，弱密碼裸奔。

2. **CORS 加公網來源**（應用層，決定「瀏覽器請求被不被接受」）
   編輯 VPS 上 `user_data/config.json` 的 `api_server.CORS_origins`，加入公網 IP 的三個 port：
   ```json
   "CORS_origins": [
       "http://<vps-ip>:8080",
       "http://<vps-ip>:8081",
...
瀏覽器開 `http://<vps-ip>:8080`（及 8081 / 8082）。
...
把 `docker-compose.yml` 的 `8080:8080` 改回 `127.0.0.1:8080:8080`（三個 service 都改），
